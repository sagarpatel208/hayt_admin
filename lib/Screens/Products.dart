          ...
          : _products.length > 0
              ? ListView.builder(
                  ...
                  itemBuilder: (BuildContext context, int index) {
                    return ProductsComponents(_products[index]);
                  })
              : Center(
...
class ProductsComponents extends StatefulWidget {
  var _product;
  ProductsComponents(this._product);
  @override
  _ProductsComponentsState createState() => _ProductsComponentsState();
}

class _ProductsComponentsState extends State<ProductsComponents> {
  ProgressDialog pr;
  @override
  void initState() {
  ...
  _makeVIPProduct() async {
    try {
      pr.show();
      final result = await InternetAddress.lookup('google.com');
      if (result.isNotEmpty && result[0].rawAddress.isNotEmpty) {
        FormData data = FormData.fromMap({"vipstatus": "1"});
        AppServices.MakeVIPProduct(widget._product["id"], data).then(
            (data) async {
          pr.hide();
          if (data.data == "0") {
  ...
  _activateProduct() async {
    try {
      pr.show();
      final result = await InternetAddress.lookup('google.com');
      if (result.isNotEmpty && result[0].rawAddress.isNotEmpty) {
        FormData data = FormData.fromMap({"status": "1"});
        AppServices.UpdateProductStatus(widget._product["id"], data).then(
            (data) async {
          pr.hide();
          if (data.data == "0") {
            Fluttertoast.showToast(
      ...
      child: Column(
        children: [
          widget._product["picture"] == null ||
                  ...
                  widget._product["picture"]["images"].length == 0 ||
                  widget._product["picture"]["images"] == null
              ? Image.asset(
