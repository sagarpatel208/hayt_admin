...
import 'package:hayt_admin/Screens/ConversationWithSeller.dart';
  ...
  _getBuyertoAdminChat() async {
    try {
      ...
      if (result.isNotEmpty && result[0].rawAddress.isNotEmpty) {
        ...
        print("iiii: ${id}");
        AppServices.GetSellerToAdminChatList(data).then((data) async {
          setState(() {
            isLoading = false;
          });
          if (data.data == "0") {
          ...
          : _chatlist.length > 0
              ? ListView.builder(
                  ...
                  itemBuilder: (BuildContext context, int index) {
                    return SellerChatListComponents(adminId, _chatlist[index]);
                  })
              : Center(
                  child: Text(
                  "No Chats available",
...
class SellerChatListComponents extends StatefulWidget {
  String _adminId;
  var _seller;
  SellerChatListComponents(this._adminId, this._seller);
  @override
  _SellerChatListComponentsState createState() =>
      _SellerChatListComponentsState();
}

class _SellerChatListComponentsState extends State<SellerChatListComponents> {
  @override
  Widget build(BuildContext context) {
    return GestureDetector(
        onTap: () {
          Navigator.push(
              context,
              MaterialPageRoute(
                  builder: (context) => ConversationWithSeller(
                        widget._adminId,
                        widget._seller["id"].toString(),
                      )));
        },
        child: Padding(
          ...
          child: Row(
            children: [
              widget._seller['logo'] == null || widget._seller['logo'] == ""
                  ? ClipOval(
              ...
              Padding(
                ...
                child: Text("${widget._seller["shopname"]}"),
              ),
            ],
          ),
        ));
